...
const DEFAULT_COLUMNS = [
  ...
  // Nhóm cột: Số giờ làm thêm thực tế
  { key: 'paidHours', label: 'Hưởng lương', visible: true, width: '100px', align: 'end' },
  { key: 'compHours', label: 'Nghỉ bù', visible: true, width: '100px', align: 'end' },
  { key: 'totalActual', label: 'Tổng cộng (Thực tế)', visible: true, width: '100px', align: 'end' },

  { key: 'maxHours', label: 'Số giờ tối đa (2)', visible: true, width: '120px', align: 'end' },
  { key: 'remaining', label: 'Số giờ còn lại (3)', visible: true, width: '150px', align: 'end' },
]
...
const PageTable = ({ data, columns }) => {
  ...
  // -- LOGIC CỘT CON (HƯỞNG LƯƠNG, NGHỈ BÙ, TỔNG CỘNG) --
  // Các cột con của nhóm "Số giờ làm thêm thực tế"
  const actualCols = ['paidHours', 'compHours', 'totalActual']
  // Tính số lượng cột con đang hiển thị để set colSpan cho header
  const visibleActualCols = columns.filter(c => actualCols.includes(c.key) && c.visible).length

  // -- LOGIC FOOTER COLSPAN --
  // Tìm index của cột số liệu đầu tiên (ví dụ paidHours) để tính gộp ô "Tổng cộng"
  const firstMetricIndex = visibleColumns.findIndex(c => ['paidHours', 'compHours', 'totalActual', 'maxHours', 'remaining'].includes(c.key))
  // Nếu không tìm thấy cột số liệu nào thì gộp hết, ngược lại gộp các cột trước nó
  ...
  // Helper function để check visibility
      ...
      <CTable hover responsive className="mb-0" small bordered>
        <CTableHead>
          <CTableRow>
            ...
            {isVisible('code') && <CTableHeaderCell rowSpan={2} className="table-header-cell sticky-col-code" style={{ width: '150px' }}>Mã nhân viên</CTableHeaderCell>}
            {isVisible('pin') && <CTableHeaderCell rowSpan={2} className="table-header-cell" style={{ width: '30px' }}><span style={{fontSize: '0.8rem'}}>📌</span></CTableHeaderCell>}
            {isVisible('name') && <CTableHeaderCell rowSpan={2} className="table-header-cell" style={{ minWidth: '150px' }}>Tên nhân viên</CTableHeaderCell>}
            ...
            {/* Nhóm cột "Số giờ làm thêm thực tế" - Chỉ hiện nếu có ít nhất 1 cột con visible */}
            {visibleActualCols > 0 && (
            ...
            {isVisible('maxHours') && <CTableHeaderCell rowSpan={2} className="table-header-cell" style={{ minWidth: '120px' }}>Số giờ làm thêm tối đa (2)</CTableHeaderCell>}
            {isVisible('remaining') && <CTableHeaderCell rowSpan={2} className="table-header-cell" style={{ minWidth: '150px' }}>Số giờ làm thêm còn lại (3)</CTableHeaderCell>}
          </CTableRow>

          {/* Hàng 2: Các cột con */}
          <CTableRow>
            {isVisible('paidHours') && <CTableHeaderCell className="table-header-cell">Hưởng lương</CTableHeaderCell>}
            {isVisible('compHours') && <CTableHeaderCell className="table-header-cell">Nghỉ bù</CTableHeaderCell>}
            {isVisible('totalActual') && <CTableHeaderCell className="table-header-cell">Tổng cộng</CTableHeaderCell>}
          </CTableRow>
        </CTableHead>
        ...
        {/* Footer - Tổng cộng (Chỉ hiện nếu có dữ liệu) */}
        {hasData && (
          <CTableFoot>
            <CTableRow>
              ...
              {/* Render các cột số liệu còn lại */}
              {visibleColumns.slice(footerColSpan).map(col => {
                 let content = ""
                 // Map giá trị tổng tương ứng với key cột
...
const OvertimeSummaryPage = () => {
  ...
  // const MOCK_DATA = [
  //   { id: 1, code: 'NV001', name: 'Nguyễn Văn A', position: 'Nhân viên', department: 'IT', paidHours: 10, compHours: 5, totalActual: 15, maxHours: 40, remaining: 25 },
  //   { id: 2, code: 'NV002', name: 'Trần Thị B', position: 'Kế toán', department: 'TC', paidHours: 0, compHours: 8, totalActual: 8, maxHours: 40, remaining: 32 },
  // ]
