...
const PageHeader = () => {
  return (
    <div className="page-header">
      <h2 className="page-title">Tổng hợp tình hình làm thêm của nhân viên</h2>
      ...
      <CTable hover responsive className="mb-0" small bordered>
        <CTableHead>
          ...
          <CTableRow>
            ...
            <CTableHeaderCell rowSpan={2} className="table-header-cell" style={{ width: '30px' }}>
            ...
            {/* Nhóm cột "Số giờ làm thêm thực tế (1)" */}
            <CTableHeaderCell colSpan={3} className="table-header-cell" style={{backgroundColor: '#ebedef'}}>
              Số giờ làm thêm thực tế (1)
            </CTableHeaderCell>

            <CTableHeaderCell rowSpan={2} className="table-header-cell" style={{ minWidth: '120px' }}>
              Số giờ làm thêm tối đa (2)
            </CTableHeaderCell>

            <CTableHeaderCell rowSpan={2} className="table-header-cell" style={{ minWidth: '150px' }}>
              Số giờ làm thêm còn lại (3) = (2) - (1)
            </CTableHeaderCell>
          </CTableRow>

          {/* Hàng 2 (Con của Số giờ làm thêm thực tế) */}
          <CTableRow>
            <CTableHeaderCell className="table-header-cell">Hưởng lương</CTableHeaderCell>
            <CTableHeaderCell className="table-header-cell">Nghỉ bù</CTableHeaderCell>
          ...
          {!hasData ? (
            ...
            data.map((item, index) => (
              <CTableRow key={item.id}>
                ...
                <CTableDataCell className="text-end font-weight-bold">{item.totalActual}</CTableDataCell>
                <CTableDataCell className="text-end">{item.maxHours}</CTableDataCell>
                <CTableDataCell className="text-end font-weight-bold text-success">{item.remaining}</CTableDataCell>
              </CTableRow>
            ))
          )}
        </CTableBody>
        ...
        <CTableFoot>
          <CTableRow>
            {/* Gộp 6 cột đầu tiên */}
            <CTableHeaderCell colSpan={6} className="table-footer-cell ps-3 text-start">
              Tổng cộng
            </CTableHeaderCell>
            ...
            <CTableHeaderCell className="table-footer-cell text-end">{totals.totalActual}</CTableHeaderCell>
            <CTableHeaderCell className="table-footer-cell text-end">{totals.maxHours}</CTableHeaderCell>
            <CTableHeaderCell className="table-footer-cell text-end">{totals.remaining}</CTableHeaderCell>
          </CTableRow>
        </CTableFoot>
      </CTable>
    </div>
