...
//       width: 80px;
//       height: 80px;
//       color: #6f42c1; /* Màu tím nhạt giống trong ảnh */
...
//       {/* Tiêu đề */}
//       <h3 className="empty-state-title">Đơn xin nghỉ</h3>

//       {/* Mô tả */}
//       <p className="empty-state-description">
//         Tổng hợp danh sách đơn xin nghỉ của nhân viên trong công ty
//       </p>
//       <CButton 
//         color="primary" 
...
//   const handleAddNew = () => {
//     navigate('/timesheet/leaveRequest/add') 
...
import { useCallback, useEffect, useRef, useState } from 'react'; // Đã thêm useCallback
import { useNavigate } from 'react-router-dom';
...
const DEFAULT_COLUMNS = [
  ...
  { key: 'department', label: 'Đơn vị', visible: true, width: 150 },
  { key: 'apply_date', label: 'Ngày nộp', visible: true, width: 140, align: 'center' },
  ...
  { key: 'approver', label: 'Người duyệt', visible: true, width: 160 },
...
// DỮ LIỆU GIẢ LẬP
const MOCK_DB = [
  { id: 1, emp_code: 'NV001', applicant: 'Nguyễn Văn A', department: 'SinhVienDungThu', apply_date: '10/12/2025', type: 'Phép năm', reason: 'Việc riêng', status: 'Đã duyệt', days_off: 1, approver: 'Lê Văn B' },
  { id: 2, emp_code: 'NV002', applicant: 'Trần Thị C', department: 'Phòng A', apply_date: '11/12/2025', type: 'Nghỉ ốm', reason: 'Khám bệnh', status: 'Chờ duyệt', days_off: 2, approver: 'Nguyễn X' },
  { id: 3, emp_code: 'NV003', applicant: 'Lê Văn D', department: 'Phòng A', apply_date: '12/12/2025', type: 'Không lương', reason: 'Ngủ quên', status: 'Từ chối', days_off: 0.5, approver: 'Nguyễn X' },
];
      ...
      <div className="filter-right" style={{ position: 'relative' }}>
        ...
        <CDropdown>
            ...
            <CDropdownMenu>
                ...
                <CDropdownItem onClick={() => onFilterChange(p => ({...p, unit: 'Phòng A'}))}>Phòng A</CDropdownItem>
                <CDropdownItem onClick={() => onFilterChange(p => ({...p, unit: 'SinhVienDungThu'}))}>SinhVienDungThu</CDropdownItem>
            </CDropdownMenu>
        </CDropdown>
...
const LeaveRequestPage = () => {
  ...
  // -- STATE MỚI: Thêm isFiltering để phân biệt loading cục bộ và toàn trang --
  ...
  // -- REF: Để tránh fetch thừa khi mount (nếu cần strict mode, ở đây giữ logic đơn giản)
  const isInitialMount = useRef(true)
  
  const fileInputRef = useRef(null)
  const navigate = useNavigate();

  // --- HÀM FETCH DATA (Sử dụng useCallback theo mẫu) ---
  const fetchData = useCallback(async (currentFilters) => {
    // Bật loading cục bộ (Spinner trong bảng)
    setIsFiltering(true);
    
    // Giả lập API call (Mô phỏng delay)
    await new Promise(r => setTimeout(r, 300));

    // Mô phỏng Server-side Filtering
    ...
    setData(result);
    setLoading(false); // Tắt loading toàn trang (nếu đang bật)
    setIsFiltering(false); // Tắt loading cục bộ
  }, []);

  // --- EFFECT: Gọi fetchData khi filters thay đổi ---
  useEffect(() => {
    fetchData(filters);
  }, [filters, fetchData]);

  // --- LOGIC RELOAD (ÁP DỤNG CHÍNH XÁC MẪU BẠN GỬI) ---
  const handleReload = () => {
    const defaultFilters = { search: '', status: 'all', unit: 'all', columnFilters: {} }
    setFilters(defaultFilters)
    setColumns(DEFAULT_COLUMNS)
    fetchData(defaultFilters) // Gọi trực tiếp để đảm bảo chạy ngay
  }

  // Logic Import/Export giữ nguyên
  ...
  const handleExportExcel = () => {
    ...
    link.href = url;
    link.setAttribute('download', 'danh_sach_don_nghi.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  // --- RENDER CONDITION: Nếu đang load lần đầu và không phải filtering ---
  ...
  return (
    <>
      ...
      <div className="page-container">
        ...
        {/* --- CHECK ISFILTERING ĐỂ HIỆN SPINNER CỤC BỘ --- */}
        {isFiltering ? (
