...
const DEFAULT_COLUMNS = [
  ...
  // ✅ CỘT NGƯỜI DUYỆT (Sẽ hiện tên)
  { key: 'approver', label: 'Người duyệt', visible: true, width: 180 },

  { key: 'startDate', label: 'Từ ngày', visible: true, width: 120, align: 'center' },
  { key: 'endDate', label: 'Đến ngày', visible: true, width: 120, align: 'center' },
  { key: 'type', label: 'Loại nghỉ', visible: true, width: 150 },
  { key: 'reason', label: 'Lý do', visible: true, width: 200 },
  { key: 'days_off', label: 'Số ngày', visible: true, width: 100, align: 'center' },
  { key: 'status', label: 'Trạng thái', visible: true, width: 140, align: 'center' },
  { key: 'actions', label: '', visible: true, width: 80 }
];

const INITIAL_FILTERS = { search: '', status: 'all', columnFilters: {} };
...
// Map tiếng Việt về code Backend để filter
...
// =====================================================================
// 1. CSS & POPUP (Giữ nguyên cho gọn)
// =====================================================================
const PageStyles = () => (
  <style>
    {`
...
const PageHeader = ({ onImportClick, onAddClick }) => (
  <div className="page-header">
    <div className="page-title">Đơn xin nghỉ</div>
    ...
    <div className="filter-bar">
      <div className="filter-left">
        <CInputGroup className="search-bar">
          ...
          <CFormInput className="border-start-0 ps-0" placeholder="Tìm kiếm (ID, Tên, Lý do...)" value={filters.search} onChange={(e) => onFilterChange((prev) => ({ ...prev, search: e.target.value }))} size="sm" />
        </CInputGroup>
      </div>
              ...
              <CTableRow key={item.id}>
                {visibleColumns.map((col) => {
                  ...
                  if (col.key === 'status') {
                    ...
                    else if (item.status === 'Bị từ chối') badgeClass += 'status-rejected';
...
const LeaveRequestPage = () => {
  ...
  // --- HÀM FETCH DATA (ĐÃ CẬP NHẬT ĐỂ HIỆN NGƯỜI DUYỆT) ---
  const fetchData = useCallback(async (currentFilters) => {
    ...
    try {
        ...
        // 2. Tạo từ điển nhân viên (Mapping ID -> Tên)
        const employeeMap = {};
        const employeeList = empRes?.data?.employees || [];
        if (employeeList.length > 0) {
            employeeList.forEach(emp => {
                // Lưu vào map theo ID, dùng String() để đảm bảo khớp key
                employeeMap[String(emp.employeeId)] = {
        ...
        // 3. Map dữ liệu để hiển thị
        if (leaveRes && leaveRes.content) {
            const mappedData = leaveRes.content.map(item => {
                // Lấy tên Người nộp
                const empInfo = employeeMap[String(item.employeeId)] || { code: 'Unknown', name: `ID: ${item.employeeId}` };
                
                // Lấy tên Người duyệt (Dùng chung map vì Người duyệt cũng là nhân viên)
                let approverName = '-';
                if (item.accountApproverId) {
                    const approverInfo = employeeMap[String(item.accountApproverId)];
                    approverName = approverInfo ? approverInfo.name : `ID: ${item.accountApproverId}`;
                }
                ...
                return {
                    ...
                    type: item.leaveType === 'PAID' ? 'Có lương' : 'Không lương',
                    reason: item.reason,
                    days_off: item.totalDays,
                    status: mapStatusToVN(item.status), 
                };
            });

            // 4. Lọc Search ở Client
            let finalResult = mappedData;
            if (currentFilters.search) {
                 const searchLower = currentFilters.search.toLowerCase();
                 finalResult = finalResult.filter(item => 
                    item.emp_code.toLowerCase().includes(searchLower) || 
                    item.applicant.toLowerCase().includes(searchLower) || 
                    item.approver.toLowerCase().includes(searchLower) || // Tìm theo tên người duyệt
                    item.reason.toLowerCase().includes(searchLower)
                 );
            }

            // 5. Lọc Advanced Column Filter
            ...
            setData(finalResult);
            setPagination({
                page: leaveRes.number,
                totalPages: leaveRes.totalPages,
                totalElements: leaveRes.totalElements
            });
        }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
      ...
      const confirmMsg = currentItem && currentItem.status !== 'Đang chờ duyệt' 
          ? `Đơn này đang "${currentItem.status}". Bạn có chắc chắn muốn hủy không?`
          : "Bạn có chắc chắn muốn xóa/hủy đơn này không?";
  ...
  const handleEdit = (id) => { navigate(`/timesheet/leaveRequest/edit/${id}`); };
  const handleReload = () => { const defaultFilters = { search: '', status: 'all', columnFilters: {} }; setFilters(defaultFilters); fetchData(defaultFilters); }
  ...
  const handleExportExcel = () => {
    ...
    data.forEach(item => { const rowData = keys.map(k => `"${item[k] || ''}"`); csvRows.push(rowData.join(',')); });
    const csvString = csvRows.join('\n'); const blob = new Blob(['\uFEFF' + csvString], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.setAttribute('download', 'danh_sach_don_nghi.csv'); document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };
  ...
  return (
    <>
      ...
      <div className="page-container">
        <PageHeader onImportClick={() => { setVisibleImportModal(true); setImportFile(null); }} onAddClick={() => navigate('/timesheet/leaveRequest/add')} />
        ...
        {isFiltering ? (<div className="d-flex justify-content-center p-5 table-wrapper-fullscreen"><CSpinner color="primary" /></div>) : (<PageTable data={data} columns={columns} onEdit={handleEdit} onDelete={handleDelete} />)}
      </div>
      <CModal alignment="center" size="lg" visible={visibleImportModal} onClose={() => setVisibleImportModal(false)}>
        <CModalHeader><CModalTitle className="fw-bold fs-5">Nhập khẩu dữ liệu</CModalTitle></CModalHeader>
        <CModalBody>
